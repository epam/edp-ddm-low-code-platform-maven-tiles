...
image::structure.svg[low-code-platform-maven-tiles structure]

[CAUTION]
Зображено лише основні кроки під час генерації RestAPI asciidoc. Кроки по переміщенню та видаленню файлів під час генерації не зображені на діаграмі. Більш детальна інформація в xref:low-code-platform-maven-tiles:implementation.adoc[описі реалізації].
...
image::document-types.svg[low-code-platform-maven-tiles document types]

=== Структура файлів

[plantuml, low-code-platform-maven-tiles-file-structure-templete, svg]
----
include::partial$puml/file-structure-template.puml[low-code-platform-maven tiles-file-structure-templete]
----
